...
namespace SVDU
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (gamestate == 0)
            {
                if (GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed && !prevA)
                {
                    gamestate = 1;
                }

                prevA = GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed;
                return;
            }
            if (gamestate == 1)
            {
            ...
            if (gamestate == 3)
            {
                if (GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y > .75 && cooldown > 0.1)
                {
                    //dec 1
                ...
                else if (GamePad.GetState(PlayerIndex.One).ThumbSticks.Left.Y < -.75 && cooldown > 0.1)
                {
                    //inc 1
                    currchar = alph.ToCharArray()[(alph.IndexOf(currchar) + 1) % 26];
                    cooldown = 0;
                }
                if (GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed && !prevA && name.Length < 8)
                {
                    name = name + currchar;
                    currchar = 'A';
                }
                if (GamePad.GetState(PlayerIndex.One).Buttons.B == ButtonState.Pressed && !prevB && name.Length > 0)
                {
                    currchar = name.ToCharArray().Last();
                    name = name.Substring(0, name.Length - 1);
                }
                if (GamePad.GetState(PlayerIndex.One).Buttons.Start == ButtonState.Pressed && prevStart == false)
                {
                    ...
                    SW.Close();
                    gamestate = 4;
                    prevStart = GamePad.GetState(PlayerIndex.One).Buttons.Start == ButtonState.Pressed;
                    name = "";
                    Score.reset();
                    return;
                }
                ...
                prevA = GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed;
                prevB = GamePad.GetState(PlayerIndex.One).Buttons.B == ButtonState.Pressed;
                prevStart = GamePad.GetState(PlayerIndex.One).Buttons.Start == ButtonState.Pressed;
            ...
            {
                if (GamePad.GetState(PlayerIndex.One).Buttons.Start == ButtonState.Pressed && !prevStart)
                {
                    objs.Clear();
                    gamestate = 0;
                    prevA = GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed;
            ...
            if (GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed && !prevA && player.isAlive())
            {
                loadLevel();
                startTime = (float)gameTime.TotalGameTime.TotalSeconds;
            }
            ...
            if (GamePad.GetState(PlayerIndex.One).Buttons.Y == ButtonState.Pressed) Camera.zoomout = Math.Min(Camera.zoomout * 1.01F, 1.2F);
            if (GamePad.GetState(PlayerIndex.One).Buttons.X == ButtonState.Pressed) Camera.zoomout = Math.Max(0.5F, Camera.zoomout / 1.01F);
            ...
            if (GamePad.GetState(PlayerIndex.One).Triggers.Right > 0.1) player.fire();
            else player.release();
